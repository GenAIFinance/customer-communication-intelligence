...
def detect_campaign_underperformance(
    ...
    # Exclude no-data campaigns (total_sent == 0) from the median baseline.
    # Including them with synthetic open_rate=0.0 drives the median toward zero,
    # which can suppress real underperformer flags when tracking is incomplete.
    campaigns_with_data = campaign_stats[campaign_stats["total_sent"] > 0]

    if campaigns_with_data.empty:
        return AnomalyResult(
            detector="campaign_underperformance",
            summary="No campaigns with open tracking data found.",
            metadata={"ratio_threshold": ratio},
        )

    median_open_rate = campaigns_with_data["open_rate"].median()
    cutoff = median_open_rate * ratio

    # Flag from all campaigns with data only — no-data campaigns are excluded
    underperforming = campaigns_with_data[
        campaigns_with_data["open_rate"] < cutoff
    ].copy()
    ...
    if flagged:
        ...
        summary = (
            f"All {len(campaigns_with_data)} campaigns performing above "
    ...
    return AnomalyResult(
        ...
        metadata={
            ...
            "campaigns_checked": len(campaigns_with_data),
        },
    )
